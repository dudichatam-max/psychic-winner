...
class MicChannelFx {
    ...
    private var cachedHpHz = Float.NaN
    private var cachedHpSr = -1
    private var cachedHpCoeff = 0f
    private var cachedPresSr = -1
    private var cachedPresCoeff = 0f

    fun process(input: Float, sampleRate: Int): Float {
        val hz = hpfHz
        if (hz != cachedHpHz || sampleRate != cachedHpSr) {
            cachedHpHz = hz
            cachedHpSr = sampleRate
            cachedHpCoeff = 1f - exp((-2.0 * Math.PI * hz.toDouble() / sampleRate).toFloat())
        }
        hpZ += cachedHpCoeff * (input - hpZ)
        var x = input - hpZ
        env = env * 0.97f + abs(x) * 0.03f
        val target = if (env > gateThresh) 1f else 0.15f
        gateOpen += 0.1f * (target - gateOpen)
        x *= gateOpen
        if (sampleRate != cachedPresSr) {
            cachedPresSr = sampleRate
            cachedPresCoeff = 1f - exp((-2.0 * Math.PI * 2800.0 / sampleRate).toFloat())
        }
        presZ += cachedPresCoeff * (x - presZ)
...
class MicCaptureEngine(private val sampleRate: Int) {
    ...
    // 70Hz / 2500Hz monitor filters. sampleRate is fixed for this engine.
    private val monHpCoeff = 1f - exp((-2.0 * Math.PI * 70.0 / sampleRate).toFloat())
    private val monPresCoeff = 1f - exp((-2.0 * Math.PI * 2500.0 / sampleRate).toFloat())
    ...
    @Volatile var lastMixL: Float = 0f
    @Volatile var lastMixR: Float = 0f
    ...
    fun playMixSample(): Float {
        ...
        for (t in 0 until trackCount) {
            ...
            val s = trackFx[t].process(raw, sampleRate)
            val fx = trackFx[t]
            l += s * fx.panL
            r += s * fx.panR
        }
        lastMixL = l.coerceIn(-1f, 1f)
        lastMixR = r.coerceIn(-1f, 1f)
        return ((lastMixL + lastMixR) * 0.5f).coerceIn(-1f, 1f)
    }
    ...
    private fun lightMonitorFx(input: Float): Float {
        monHpZ += monHpCoeff * (input - monHpZ)
        var x = input - monHpZ
        monPresZ += monPresCoeff * (x - monPresZ)
