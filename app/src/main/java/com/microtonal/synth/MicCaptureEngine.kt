...
class MicChannelFx {
    @Volatile var hpfHz: Float = 80f
    @Volatile var gateThresh: Float = 0.012f
    @Volatile var presenceGain: Float = 1.1f
    @Volatile var compAmount: Float = 0.25f
...
class MonitorPresetSlot {
    ...
    var monitorVolume: Float = 0.85f
    var hpfHz: Float = 90f
    var gateThresh: Float = 0.015f
    var lowGain: Float = 1.0f
    var presenceGain: Float = 1.15f
    var compAmount: Float = 0.3f
}
...
class MicCaptureEngine(private val sampleRate: Int) {
    ...
    @Volatile var monitorVolume: Float = 0.85f
    @Volatile var headphonesConnected: Boolean = false

    @Volatile var hpfHz: Float = 90f
    @Volatile var gateThresh: Float = 0.015f
    @Volatile var lowGain: Float = 1.0f
    @Volatile var presenceGain: Float = 1.15f
    @Volatile var compAmount: Float = 0.3f
        ...
        captureThread = Thread({
            ...
            while (capturing) {
                ...
                for (i in 0 until n) {
                    ...
                    energy += raw * raw
                    seen++
                    val processed = processLiveFx(raw)
                    val w = ringWrite
                    ring[w] = processed
                    ringWrite = (w + 1) % ring.size
                    monitorVisualizer[monVisWrite] = processed
