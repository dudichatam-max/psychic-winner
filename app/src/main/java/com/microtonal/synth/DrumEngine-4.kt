...
class DrumEngine(private val sampleRate: Int) {
    ...
    private fun decodeAudio(context: Context, uri: Uri): FloatArray? {
        // 1. Try native Android MediaExtractor first (WAV, MP3, FLAC, AAC, OGG…)
        val mediaResult = decodeWithMediaExtractor(context, uri)
        if (mediaResult != null) return mediaResult

        // 2. Fallback: custom AIFF / AIF parser (Android has no native AIFF support)
        return decodeAiff(context, uri)
    }

    /** Standard MediaExtractor + MediaCodec path (existing logic). */
    private fun decodeWithMediaExtractor(context: Context, uri: Uri): FloatArray? {
        ...
        try {
            ...
            return resampleToEngineRate(raw, rawSize, fileSampleRate)
        } catch (e: Exception) {
            e.printStackTrace()
            return null
        } finally {
    ...
    /**
     * Lightweight AIFF / AIF / AIFC (uncompressed PCM only) parser.
     * Supports 8/16/24/32-bit integer PCM, mono or stereo.
     * Converts to mono FloatArray and resamples to engine sampleRate.
     */
    private fun decodeAiff(context: Context, uri: Uri): FloatArray? {
        return try {
            context.contentResolver.openInputStream(uri)?.use { input ->
                ...
                if (header[0] != 'F'.code.toByte() || header[1] != 'O'.code.toByte() ||
                    header[2] != 'R'.code.toByte() || header[3] != 'M'.code.toByte()) return null

                val formType = String(header, 8, 4, Charsets.US_ASCII)
                if (formType != "AIFF" && formType != "AIFC") return null

                var numChannels = 1
                var numSampleFrames = 0
                var sampleSize = 16
                var fileSampleRate = 44100.0
                var soundData: ByteArray? = null
                var isLittleEndian = false // true for AIFC "sowt"

                // Parse all chunks (COMM can appear after SSND in rare files)
                ...
                while (true) {
                    ...
                    if (chunkSize < 0 || chunkSize > 200_000_000) break // safety

                    when (chunkId) {
                        "COMM" -> {
                            val toRead = chunkSize.coerceAtMost(128)
                            val comm = ByteArray(toRead)
                            val got = input.read(comm)
                            if (got < 18) return null
                            val bb = ByteBuffer.wrap(comm).order(ByteOrder.BIG_ENDIAN)
                            numChannels = bb.short.toInt().coerceIn(1, 8)
                            numSampleFrames = bb.int
                            sampleSize = bb.short.toInt()
                            // 80-bit IEEE extended → double (offset 8 inside COMM data)
                            fileSampleRate = extended80ToDouble(comm, 8)
                            if (fileSampleRate < 1000.0 || fileSampleRate > 384000.0) {
                                fileSampleRate = 44100.0
                            }
                            // AIFC compression type starts at byte 18 of COMM
                            if (formType == "AIFC" && got >= 22) {
                                val comp = String(comm, 18, 4, Charsets.US_ASCII)
                                when (comp) {
                                    "NONE", "raw ", "twos" -> isLittleEndian = false
                                    "sowt" -> isLittleEndian = true   // little-endian PCM (common on Mac)
                                    else -> return null              // compressed (not supported)
                                }
                            }
                            val remaining = chunkSize - got
                            if (remaining > 0) input.skip(remaining.toLong())
                            if (chunkSize % 2 != 0) input.skip(1)
                        }
                        "SSND" -> {
                            if (chunkSize < 8) return null
                            val offsetAndBlock = ByteArray(8)
                            if (input.read(offsetAndBlock) != 8) return null
                            val ssndOffset = ByteBuffer.wrap(offsetAndBlock).order(ByteOrder.BIG_ENDIAN).int
                            val dataSize = chunkSize - 8
                            if (dataSize <= 0) return null
                            // Skip the optional offset inside the sound data
                            if (ssndOffset > 0) {
                                input.skip(ssndOffset.toLong())
                            }
                            val actualDataSize = dataSize - ssndOffset.coerceAtLeast(0)
                            if (actualDataSize <= 0) return null
                            soundData = ByteArray(actualDataSize)
                            var pos = 0
                            while (pos < actualDataSize) {
                                val r = input.read(soundData!!, pos, actualDataSize - pos)
                                if (r <= 0) break
                                pos += r
                            }
                            if (pos < actualDataSize) soundData = soundData!!.copyOf(pos)
                            if (chunkSize % 2 != 0) input.skip(1)
                        }
                        else -> {
                ...
                if (soundData == null || numSampleFrames <= 0 || sampleSize !in listOf(8, 16, 24, 32)) {
                    return null
                }

                val bytesPerSample = sampleSize / 8
                val channels = numChannels.coerceIn(1, 8)
                val expectedBytes = numSampleFrames * channels * bytesPerSample
                val available = minOf(soundData.size, expectedBytes)
                val frames = available / (channels * bytesPerSample)
                if (frames <= 0) return null

                val raw = FloatArray(frames)
                val order = if (isLittleEndian) ByteOrder.LITTLE_ENDIAN else ByteOrder.BIG_ENDIAN
                val bb = ByteBuffer.wrap(soundData, 0, available).order(order)

                for (i in 0 until frames) {
                    var sample = 0f
                    for (ch in 0 until channels) {
                        val v = when (sampleSize) {
                            8 -> {
                                val b = bb.get().toInt() and 0xFF
                                (b - 128) / 128f          // unsigned 8-bit → signed
                            }
                            16 -> bb.short / 32768f
                            24 -> {
                                ...
                                val s = if (isLittleEndian) {
                                    // little-endian 24-bit
                                    var v24 = b0 or (b1 shl 8) or (b2 shl 16)
                                    if (v24 and 0x800000 != 0) v24 = v24 or -0x1000000
                                    v24
                                } else {
                                    // big-endian 24-bit
                                    var v24 = (b0 shl 16) or (b1 shl 8) or b2
                                    if (v24 and 0x800000 != 0) v24 = v24 or -0x1000000
                                    v24
                                }
                                s / 8388608f
                    ...
                    raw[i] = if (channels > 1) sample / channels else sample
                }

                return resampleToEngineRate(raw, frames, fileSampleRate.toInt().coerceIn(8000, 384000))
            }
        } catch (e: Exception) {
            e.printStackTrace()
            null
    ...
    /** Convert IEEE 80-bit extended precision (AIFF sample rate) to Double. */
    private fun extended80ToDouble(data: ByteArray, offset: Int): Double {
        if (offset + 10 > data.size) return 44100.0
        val exp = ((data[offset].toInt() and 0x7F) shl 8) or (data[offset + 1].toInt() and 0xFF)
        val mantissaHi = ((data[offset + 2].toInt() and 0xFF).toLong() shl 24) or
                ((data[offset + 3].toInt() and 0xFF).toLong() shl 16) or
                ((data[offset + 4].toInt() and 0xFF).toLong() shl 8) or
                (data[offset + 5].toInt() and 0xFF).toLong()
        val mantissaLo = ((data[offset + 6].toInt() and 0xFF).toLong() shl 24) or
                ((data[offset + 7].toInt() and 0xFF).toLong() shl 16) or
                ((data[offset + 8].toInt() and 0xFF).toLong() shl 8) or
                (data[offset + 9].toInt() and 0xFF).toLong()

        if (exp == 0 && mantissaHi == 0L && mantissaLo == 0L) return 0.0

        val sign = if ((data[offset].toInt() and 0x80) != 0) -1.0 else 1.0
        // 80-bit: 1 sign + 15 exp + 64 mantissa (no hidden bit in the same way)
        val mantissa = (mantissaHi.toDouble() * Math.pow(2.0, 32.0) + mantissaLo.toDouble()) / Math.pow(2.0, 63.0)
        return sign * mantissa * Math.pow(2.0, (exp - 16383).toDouble())
    }

    /** Shared linear resampling helper used by both Media and AIFF paths. */
    private fun resampleToEngineRate(raw: FloatArray, rawSize: Int, fileSampleRate: Int): FloatArray {
        if (fileSampleRate <= 0 || rawSize <= 0) return FloatArray(0)
