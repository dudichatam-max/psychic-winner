...
internal fun buildConsoleSessionJson(
    ...
    o.put("wah", wahVal.toDouble())
    o.put("oct", octVal.toDouble())
    o.put("cho", choVal.toDouble())
    return o
}
...
internal fun parseSessionUi(o: JSONObject): SessionUi {
    ...
    return SessionUi(
        ...
        oct = o.optDouble("oct", 0.0).toFloat(),
        cho = o.optDouble("cho", 0.0).toFloat()
    )
}
...
internal fun restoreEngineFromSession(
    ...
    val looperDir = sessionFolder?.let { File(it, "looper") }
    val meta = if (looperDir != null && looperDir.isDirectory) engine.loadLooperSession(looperDir) else engine.loadLooperSession()
    if (meta != null) {
    ...
    val drumsDir = sessionFolder?.let { File(it, "drums") }
    if (drumsDir != null && drumsDir.isDirectory) {
        engine.drumEngine.loadKitBundleFromDir(context, drumsDir)
    ...
    sessionFolder?.let { folder ->
        val presetFile = File(folder, "presets.json")
        if (presetFile.exists()) applyPresetSnapshot(prefs, JSONObject(presetFile.readText()))
        val micDir = File(folder, "mic")
        if (micDir.isDirectory) {
            val micMeta = engine.loadMicSession(micDir)
            if (micMeta != null) {
                micMeta.channelNames.forEachIndexed { i, n -> if (i < micNames.size) micNames[i] = n }
                micMeta.volumes.forEachIndexed { i, v ->
                ...
                micMeta.pans.forEachIndexed { i, p ->
...
internal fun persistFullSessionBundle(
    context: Context,
    engine: SynthEngine,
    prefs: SharedPreferences,
    name: String,
    json: JSONObject,
    looperProject: String,
    ...
    return try {
        engine.drumEngine.saveCurrentToPattern(selectedPattern)
        PresetManager.saveAllDrumKits(prefs, engine)
        engine.saveLooperSession(looperProject, loopNames, looperPage)
        val folder = PresetManager.sessionFolder(context, name)
        if (!folder.exists()) folder.mkdirs()
        engine.saveLooperSession(looperProject, loopNames, looperPage, File(folder, "looper"))
        engine.saveMicSession(micProject, micNames, micPage, File(folder, "mic"))
        engine.drumEngine.writeKitBundleToDir(context, File(folder, "drums"))
        File(folder, "presets.json").writeText(snapshotPresets(prefs).toString())
        PresetManager.saveSession(context, name, json)
    } catch (e: Exception) {
        e.printStackTrace()
        false
    }
...
internal fun snapshotPresets(prefs: SharedPreferences): JSONObject {
    val o = JSONObject()
    val all = prefs.all
    for ((k, v) in all) {
        if (!(k.startsWith("p_") || k.startsWith("page_"))) continue
        when (v) {
            is String -> o.put(k, v)
...
internal fun applyPresetSnapshot(prefs: SharedPreferences, o: JSONObject) {
    val ed = prefs.edit()
    val keys = o.keys()
    while (keys.hasNext()) {
        val k = keys.next()
        ...
        when (v) {
            ...
            is Number -> {
                when {
                    k.endsWith("_wave") || k.endsWith("_octave") -> ed.putInt(k, v.toInt())
                    k.endsWith("_exists") -> ed.putBoolean(k, v.toInt() != 0)
