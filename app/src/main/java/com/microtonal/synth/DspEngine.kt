...
class DspEngine(private val sampleRate: Int = 44100) {
    ...
    /** Additive piano body. Partials use their own phases so wrapping the fundamental cannot click. */
    private fun generatePianoWave(
        ...
        val bright = (freq / 1100.0).coerceIn(0.20, 1.0)
        val decayBright = bright * (0.25 + 0.75 * env)
        val s1 = fastSine(p1)
        val s2 = fastSine(p2)
        val s3 = fastSine(p3)
        val s4 = fastSine(p4)
        var out = s1 * 0.38 + s2 * (0.18 * decayBright) + s3 * (0.09 * decayBright) + s4 * (0.04 * decayBright)
        if (envState == 0 && env < 0.85) {
            out += fastSine(p3) * (0.85 - env) * (0.08 / bright.coerceAtLeast(0.4))
        }
        if (out > 0.55) out = 0.55
        else if (out < -0.55) out = -0.55
        return out
    }
    ...
    fun processNextSample(
        ...
        for (v in 0 until maxVoices) {
            ...
            if (usePiano) {
                slot.phaseP2 += 2.0 * PI * dt * 2.002
                slot.phaseP3 += 2.0 * PI * dt * 3.008
                slot.phaseP4 += 2.0 * PI * dt * 4.018
            ...
            if (detuneOn) {
                ...
                val raw2 = if (usePiano) {
                    generatePianoWave(
                        phaseNorm2,
                        wrap01(slot.phaseP2 / (2.0 * PI) + 0.02),
                        wrap01(slot.phaseP3 / (2.0 * PI) + 0.03),
                        wrap01(slot.phaseP4 / (2.0 * PI) + 0.04),
                        slot.currentFreq * 1.0035f,
                        slot.envelopeVolume,
                        slot.envState
                    )
                } else {
