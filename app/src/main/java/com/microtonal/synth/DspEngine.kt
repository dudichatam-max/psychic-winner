...
class DspEngine(private val sampleRate: Int = 44100) {
    ...
    fun processNextSample(
        ...
        for (v in 0 until maxVoices) {
            ...
            // Performance Pad (LFO + Res) applies to BOTH live and looper notes
            // so recorded Pad automation is audible during loop playback.
            var targetCutoff = (if (slot.isLooperNote) slot.frozenCutoff else cutoffFreq).coerceIn(20f, 16000f)
            
            if (smoothedPerfX > 0.001f) {
            ...
            var voiceSample = raw * slot.envelopeVolume * slot.velocity * currentHeadroom * 0.5 * resGainComp
