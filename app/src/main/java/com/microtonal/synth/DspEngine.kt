...
class DspEngine(private val sampleRate: Int = 44100) {
    ...
    fun processNextSample(
        ...
        val liveN = activeCount.coerceAtLeast(1)
        val targetPianoPoly = if (pianoOn && liveN > 1) 1.0 / sqrt(1.0 + (liveN - 1) * 0.65) else 1.0
        pianoPolyScale += (targetPianoPoly - pianoPolyScale) * 0.02
        ...
        for (v in 0 until maxVoices) {
            ...
            if (!slot.isLooperNote && (div2Wet > 0.0005 || div3Wet > 0.0005 || div4Wet > 0.0005)) {
                ...
                slot.prevFund = fund
                raw = raw * 0.88 + slot.div2 * (0.16 * div2Wet) + slot.div3 * (0.13 * div3Wet) + slot.div4 * (0.11 * div4Wet)
            }
            ...
            if (livePiano) voiceSample *= pianoPolyScale
        ...
        val liveAbs = abs(liveMix)
        if (liveAbs > 0.72) {
            val shaped = 0.72 + 0.20 * (liveAbs - 0.72) / (0.20 + (liveAbs - 0.72))
            liveMix = sign(liveMix) * shaped
        }
        if (warmOn) {
