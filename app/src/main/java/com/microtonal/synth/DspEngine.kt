...
class DspEngine(private val sampleRate: Int = 44100) {
    ...
    /** Additive piano body for live voices. LUT sines only — no Math.sin in the sample loop. */
    private fun generatePianoWave(phaseNorm: Double, freq: Float, env: Double, envState: Int): Double {
        val bright = (freq / 900.0).coerceIn(0.18, 1.2)
        val decayBright = bright * (0.35 + 0.65 * env)
        val s1 = fastSine(wrap01(phaseNorm))
        val s2 = fastSine(wrap01(phaseNorm * 2.003))
        val s3 = fastSine(wrap01(phaseNorm * 3.011))
        val s4 = fastSine(wrap01(phaseNorm * 4.028))
        var out = s1 * 0.52 + s2 * (0.30 * decayBright) + s3 * (0.16 * decayBright) + s4 * (0.08 * decayBright)
        if (envState == 0) {
            val n = wrap01(phaseNorm * 23.17)
            out += (n - 0.5) * (1.0 - env) * (0.20 / bright.coerceAtLeast(0.35))
        }
        return out
    }
    ...
    fun processNextSample(
        ...
        for (v in 0 until maxVoices) {
            ...
            val usePiano = pianoOn && !slot.isLooperNote
            var raw = if (usePiano) {
                generatePianoWave(phaseNorm, slot.currentFreq, slot.envelopeVolume, slot.envState)
            } else {
            ...
            if (detuneOn) {
                ...
                val raw2 = if (usePiano) {
                    generatePianoWave(phaseNorm2, slot.currentFreq * 1.0035f, slot.envelopeVolume, slot.envState)
                } else {
