...
class DspEngine(private val sampleRate: Int = 44100) {
    ...
    // Precomputed ~180 Hz one-pole coeff. Never call Math.exp in the sample loop.
    private val warmLpCoeff = (2.0 * PI * 180.0 * invSampleRate).coerceIn(0.001, 0.25)
    private var ripLpState = 0.0
    private var ripWet = 0.0
    private val ripLpCoeff = (2.0 * PI * 320.0 * invSampleRate).coerceIn(0.001, 0.35)
    ...
    private fun generatePianoWave(
        ...
        val bright = (freq / 1200.0).coerceIn(0.18, 1.0)
        val e2 = env * env
        val e3 = e2 * env
        val e4 = e3 * env
        var out = fastSine(p1) * (0.36 * env)
        out += fastSine(p2) * (0.16 * e2 * bright)
        out += fastSine(p3) * (0.08 * e3 * bright)
        out += fastSine(p4) * (0.04 * e4 * bright)
        if (hammer > 0.001) {
            val hamAmt = hammer * hammer * (0.06 / bright.coerceAtLeast(0.45))
            out += fastSine(p3) * hamAmt
        }
        return out
    }

    private fun softCeil(x: Double): Double {
        val lim = 0.45
        return lim * x / (lim + abs(x))
    }
    ...
    fun processNextSample(
        ...
        for (v in 0 until maxVoices) {
            ...
            if (livePiano) {
                if (slot.hammerEnv > 0.0001) slot.hammerEnv *= 0.994
                else slot.hammerEnv = 0.0
                val stretch = 0.0006 + (90.0 / slot.currentFreq.coerceAtLeast(80f)) * 0.0035
                val r2 = 2.0 * (1.0 + stretch)
                val r3 = 3.0 * (1.0 + stretch * 1.35)
                val r4 = 4.0 * (1.0 + stretch * 1.75)
                slot.phaseP2 += 2.0 * PI * dt * r2
                slot.phaseP3 += 2.0 * PI * dt * r3
                slot.phaseP4 += 2.0 * PI * dt * r4
                if (slot.phaseP2 >= 2.0 * PI) slot.phaseP2 %= (2.0 * PI)
                if (slot.phaseP3 >= 2.0 * PI) slot.phaseP3 %= (2.0 * PI)
                if (slot.phaseP4 >= 2.0 * PI) slot.phaseP4 %= (2.0 * PI)
            }
            val waveRaw = generateOptimizedWaveform(slot.waveform, phaseNorm, dt, invDt)
            var raw = if (livePiano) {
                val pianoRaw = generatePianoWave(
                    phaseNorm,
                    slot.phaseP2 / (2.0 * PI),
                    slot.phaseP3 / (2.0 * PI),
                    slot.phaseP4 / (2.0 * PI),
            ...
            if (detuneOn) {
                ...
                val raw2 = if (livePiano) {
                    val stretch2 = 0.0006 + (90.0 / (slot.currentFreq.toDouble() * detuneRatio).coerceAtLeast(80.0)) * 0.0035
                    slot.phase2P2 += 2.0 * PI * dt2 * (2.0 * (1.0 + stretch2))
                    slot.phase2P3 += 2.0 * PI * dt2 * (3.0 * (1.0 + stretch2 * 1.35))
                    slot.phase2P4 += 2.0 * PI * dt2 * (4.0 * (1.0 + stretch2 * 1.75))
                    if (slot.phase2P2 >= 2.0 * PI) slot.phase2P2 %= (2.0 * PI)
                    if (slot.phase2P3 >= 2.0 * PI) slot.phase2P3 %= (2.0 * PI)
                    if (slot.phase2P4 >= 2.0 * PI) slot.phase2P4 %= (2.0 * PI)
                    val piano2 = generatePianoWave(
                        phaseNorm2,
                        slot.phase2P2 / (2.0 * PI),
                        slot.phase2P3 / (2.0 * PI),
                        slot.phase2P4 / (2.0 * PI),
                        slot.currentFreq * detuneRatio.toFloat(),
                        slot.envelopeVolume,
                        slot.hammerEnv
                    )
                    val wave2 = generateOptimizedWaveform(slot.waveform, phaseNorm2, dt2, invDt2)
                    wave2 * (1.0 - pianoWet) + piano2 * pianoWet
                } else {
                    generateOptimizedWaveform(slot.waveform, phaseNorm2, dt2, invDt2)
                }
                raw = raw * 0.68 + raw2 * 0.32
            }

            if (livePiano) raw = softCeil(raw)
        ...
        if (warmOn) {
            // Cheap analog warmth: one-pole low shelf only (adds, multiplies).
            // No Math.exp / x*x here — those caused underruns and periodic
            // distortion on weak phones (x^2 also injected DC into the bus).
            warmLpState += (liveMix - warmLpState) * warmLpCoeff
            liveMix = liveMix * 0.84 + warmLpState * 0.42
            if (liveMix > 1.2) liveMix = 1.2 else if (liveMix < -1.2) liveMix = -1.2
        } else if (warmLpState != 0.0) {
            warmLpState *= 0.99
        }
        ...
        ripWet += ((if (ripOn) 1.0 else 0.0) - ripWet) * 0.0008
        if (ripWet > 0.0005) {
            ripLpState += (liveMix - ripLpState) * ripLpCoeff
            val hp = liveMix - ripLpState
            val ripped = hp * 1.18 + ripLpState * 0.12
        ...
        phazWet += ((if (phazOn) 1.0 else 0.0) - phazWet) * 0.0008
        if (phazWet > 0.0005) {
            phazLfoPhase += 0.28 * invSampleRate
            if (phazLfoPhase >= 1.0) phazLfoPhase -= 1.0
            val lfo = fastSine(phazLfoPhase)
            val a = 0.22 + 0.55 * (0.5 + 0.5 * lfo)
            val y1 = -liveMix + a * (liveMix - phazZ1)
            phazZ1 = y1
            val y2 = -y1 + a * (y1 - phazZ2)
            phazZ2 = y2
            liveMix = liveMix * (1.0 - phazWet * 0.55) + y2 * (phazWet * 0.55)
        } else if (phazZ1 != 0.0 || phazZ2 != 0.0) {
            phazZ1 *= 0.99
            phazZ2 *= 0.99
        }
