...
class SynthEngine(private val context: Context) {
        ...
        return try {
            context.contentResolver.openOutputStream(destinationUri)?.use { os ->
                ZipOutputStream(os).use { zos ->
                    ...
                    val vols = JSONArray()
                    for (t in 0 until looperTrackCount) {
                        names.put(channelNames.getOrNull(t) ?: "ערוץ ${t + 1}")
    ...
    fun importLooperProjectFromZip(context: Context, sourceUri: Uri): LooperProjectMeta? {
        return try {
            ...
            val channelNames = MutableList(looperTrackCount) { "ערוץ ${it + 1}" }
            ...
            LooperProjectMeta(projectName, channelNames, volumes)
        } catch (e: Exception) {
            e.printStackTrace()
            null
        }
