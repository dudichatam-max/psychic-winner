        ...
        Thread {
            ...
            while (isRunning) {
                ...
                for (i in buffer.indices) {
                    ...
                    var liveOut = frame.masterSample
                    var drumOut = shapePadStem(drumSample, padTargetDrum, padCoeff, drumLp, drumHp, drumWet)
                    var loopOut = shapePadStem(pcmLoopMix, padTargetLoop, padCoeff, loopLp, loopHp, loopWet)
                    var micOut = shapePadStem(micPlay, padTargetMic, padCoeff, micLp, micHp, micWet)
                    ...
                    val gate = (stutterGain * stopGain).coerceIn(0f, 1f)
                    if (padTargetKey) liveOut *= gate
                    if (padTargetDrum) drumOut *= gate
                    if (padTargetLoop) loopOut *= gate
                    if (padTargetMic) micOut *= gate
