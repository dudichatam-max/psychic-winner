...
class SynthEngine(private val context: Context) {
    ...
    fun start() {
        ...
        Thread {
            ...
            val warmBuf = ShortArray(bufferSize * 2)
            var warm = 0
            while (warm < 44) {
                if (warm == 24) {
                    val slot = noteSlots[0]
                    slot.active = true
                    slot.isReleasing = false
                    slot.baseFreq = 220f
                    slot.targetFreq = 220f
                    slot.currentFreq = 220f
                    slot.envelopeVolume = 0.4
                    slot.waveform = 0
                    slot.isLooperNote = false
                } else if (warm == 36) {
                    noteSlots[0].isReleasing = true
                }
                dspEngine.processNextSample(noteSlots, maxVoices, 0f)
                drumEngine.processNextSample()
                java.util.Arrays.fill(warmBuf, 0.toShort())
                audioTrack.write(warmBuf, 0, warmBuf.size)
                warm++
            }
            noteSlots[0].active = false
            noteSlots[0].isReleasing = false
            noteSlots[0].envelopeVolume = 0.0
            liveLp[0] = 0f
            drumLp[0] = 0f
            drumLpR[0] = 0f
            loopLp[0] = 0f
            ...
            while (isRunning) {
                ...
                for (i in 0 until bufferSize) {
                    ...
                    var micOutR: Float
                    if (padWork) {
                        liveOut = rollLive.process(frame.masterSample, rollOn && padTargetKey, busStutterDiv, bpmNow, sampleRate, rollWetCoeff)
