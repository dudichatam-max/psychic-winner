        ...
        Thread {
            ...
            while (isRunning) {
                ...
                for (i in 0 until bufferSize) {
                    ...
                    var rawL = liveOut * center + drumOutL + loopOutL + micOutL + micMon * center
                    var rawR = liveOut * center + drumOutR + loopOutR + micOutR + micMon * center
                    val absL = abs(rawL)
                    val absR = abs(rawR)
                    limEnvL = if (absL > limEnvL) absL else limEnvL * limRelease
                    limEnvR = if (absR > limEnvR) absR else limEnvR * limRelease
                    val gL = if (limEnvL > limThresh) limThresh / limEnvL else 1f
                    val gR = if (limEnvR > limThresh) limThresh / limEnvR else 1f
                    rawL *= gL
                    rawR *= gR
