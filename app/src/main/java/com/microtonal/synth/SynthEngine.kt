...
class SynthEngine(private val context: Context) {
    ...
    private val recLp = floatArrayOf(0f)
    private val recWet = floatArrayOf(0f)
                ...
                for (i in 0 until bufferSize) {
                    ...
                    while (h < looperHotN) {
                        ...
                        if (track.isRecording) {
                            if (!recShaped) {
                                recSample = shapePadStem(recSample, padTargetKey, recLp, recWet)
                                if (padTargetKey) recSample *= stopGain
                                recShaped = true
                            }
                            track.pushSample(recSample)
                        }
                        h++
                    }
