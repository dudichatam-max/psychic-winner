...
class SynthEngine(private val context: Context) {
    ...
    /** When true, XY pad modulates looper / mic / drums instead of the live keyboard. */
    @Volatile var padFlipOn: Boolean = false
    ...
    private var busLpZ = 0f
    private var busHpZ = 0f
    private var busWet = 0f
            ...
            while (isRunning) {
                ...
                for (i in buffer.indices) {
                    val frame = dspEngine.processNextSample(
                        ...
                        performanceX = if (padFlipOn) 0f else performanceX,
                        performanceY = if (padFlipOn) 0f else performanceY,
                        loopPerformanceX = if (padFlipOn) busPadX else loopPerformanceX,
                        loopPerformanceY = if (padFlipOn) busPadY else loopPerformanceY,
                    ...
                    var busMix = drumSample + pcmLoopMix + micPlay
                    val wantWet = if (padFlipOn && busPadTouched) 1f else 0f
                    busWet += (wantWet - busWet) * 0.012f
                    if (busWet < 0.0008f) {
                        busWet = 0f
                        busLpZ = busMix
                        busHpZ = busMix
                    } else {
                        val cutoff = 400f + busPadX * 7600f
                        val coeff = 1f - kotlin.math.exp((-2.0 * Math.PI * cutoff / sampleRate).toFloat())
                        busLpZ += coeff * (busMix - busLpZ)
                        busHpZ += coeff * (busMix - busHpZ)
                        val filtered = busLpZ + (busMix - busHpZ) * (busPadY * 0.75f)
                        val resBoost = 1f + busPadY * 0.35f
                        val wet = (filtered * resBoost).coerceIn(-1.2f, 1.2f)
                        busMix = busMix * (1f - busWet) + wet * busWet
                    }
