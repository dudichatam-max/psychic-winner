...
class SynthEngine(private val context: Context) {
        ...
        Thread {
            ...
            while (isRunning) {
                ...
                for (i in 0 until bufferSize) {
                    ...
                    var drumOutL = rollDrumL.process(drumL0, drumRollDiv >= 4, drumRollDiv, bpmNow, sampleRate)
                    var drumOutR = rollDrumR.process(drumR0, drumRollDiv >= 4, drumRollDiv, bpmNow, sampleRate)
                    var loopOutL = rollLoopL.process(pcmLoopL, rollOn && padTargetLoop, busStutterDiv, bpmNow, sampleRate)
                    var loopOutR = rollLoopR.process(pcmLoopR, rollOn && padTargetLoop, busStutterDiv, bpmNow, sampleRate)
                    var micOutL = rollMicL.process(micL0, rollOn && padTargetMic, busStutterDiv, bpmNow, sampleRate)
                    var micOutR = rollMicR.process(micR0, rollOn && padTargetMic, busStutterDiv, bpmNow, sampleRate)
                    ...
                    val rawL = (liveOut * center + drumOutL + loopOutL + micOutL + micMon * center).coerceIn(-1.0f, 1.0f)
                    val rawR = (liveOut * center + drumOutR + loopOutR + micOutR + micMon * center).coerceIn(-1.0f, 1.0f)
                    val shortL = (rawL * Short.MAX_VALUE * 0.85f).toInt().coerceIn(-32768, 32767).toShort()
                    val shortR = (rawR * Short.MAX_VALUE * 0.85f).toInt().coerceIn(-32768, 32767).toShort()
                    ...
                    looperVisualizerBuffer[i] = (pcmLoopL + pcmLoopR) * 0.5f
                    drumVisualizerBuffer[i] = drumSample
                    if (recOn) {
                        byteBuffer.putShort(shortL)
                        byteBuffer.putShort(shortR)
                    }
    ...
    fun setTrackPan(trackIndex: Int, pan: Float) {
        looperTracks.getOrNull(trackIndex)?.setPan(pan)
    }
    ...
    fun importMicProjectFromZip(context: Context, sourceUri: Uri): MicProjectMeta? {
        return try {
            ...
            for (t in 0 until n) {
                ...
                micEngine.trackFx[t].setPan(pans[t])
            }
