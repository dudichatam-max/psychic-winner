...
class SynthEngine(private val context: Context) {
    ...
    private fun shapePadStem(
        ...
        val span = ((absY - 0.12f) / 0.88f).coerceIn(0f, 1f)
        val depth = if (!want) {
            0f
        } else if (padCy < 0f) {
            span * span * 0.85f
        ...
        Thread {
            ...
            while (isRunning) {
                ...
                for (i in 0 until bufferSize) {
                    ...
                    // Play already-captured takes first. Record happens after pad
                    // shape so XY / roll / STOP on KEY are frozen into the take.
                    var pcmLoopL = 0f
                    var pcmLoopR = 0f
                    var h = 0
                    while (h < looperHotN) {
                    ...
                    val liveNeed = (rollOn && padTargetKey) || (busPadTouched && padTargetKey) ||
                        rollLive.isHot() || liveWet[0] > 0.0008f
                    val drumNeed = drumRollDiv >= 4 || (busPadTouched && padTargetDrum) ||
                        rollDrumL.isHot() || rollDrumR.isHot() ||
                        drumWet[0] > 0.0008f || drumWetR[0] > 0.0008f
                    val loopNeed = (rollOn && padTargetLoop) || (busPadTouched && padTargetLoop) ||
                        rollLoopL.isHot() || rollLoopR.isHot() ||
                        loopWet[0] > 0.0008f || loopWetR[0] > 0.0008f
                    val micNeed = (rollOn && padTargetMic) || (busPadTouched && padTargetMic) ||
