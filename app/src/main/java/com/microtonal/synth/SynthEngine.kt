...
class SynthEngine(private val context: Context) {
    ...
    private fun shapePadStem(
        ...
        val target = if (want && busPadTouched) 1f else 0f
        wetArr[0] += (target - wetArr[0]) * 0.012f
        if (wetArr[0] < 0.0008f) {
            wetArr[0] = 0f
            lp[0] = input
            hp[0] = input
        ...
        lp[0] += coeff * (input - lp[0])
        hp[0] += coeff * (input - hp[0])
        val filtered = lp[0] + (input - hp[0]) * (busPadY * 0.75f)
        val wet = (filtered * (1f + busPadY * 0.35f)).coerceIn(-1.2f, 1.2f)
            ...
            while (isRunning) {
                ...
                for (i in buffer.indices) {
                    val frame = dspEngine.processNextSample(
                        ...
                        performanceX = if (padTargetKey) performanceX else 0f,
                        performanceY = if (padTargetKey) performanceY else 0f,
                        loopPerformanceX = if (padTargetLoop && busPadTouched) busPadX else 0f,
                        loopPerformanceY = if (padTargetLoop && busPadTouched) busPadY else 0f,
                    ...
                    val padCoeff = 1f - kotlin.math.exp((-2.0 * Math.PI * (400.0 + busPadX * 7600.0) / sampleRate).toFloat())
                    val drumOut = shapePadStem(drumSample, padTargetDrum, padCoeff, drumLp, drumHp, drumWet)
                    val loopOut = shapePadStem(pcmLoopMix, padTargetLoop, padCoeff, loopLp, loopHp, loopWet)
                    val micOut = shapePadStem(micPlay, padTargetMic, padCoeff, micLp, micHp, micWet)
                    var busMix = drumOut + loopOut + micOut
                    val fadeCoeff = 1f - kotlin.math.exp((-1.0 / (0.006 * sampleRate)).toFloat())
                    val stopTarget = if (busHoldStop) 0f else 1f
                    stopGain += (stopTarget - stopGain) * fadeCoeff
                    var stutTarget = 1f
                    if (busStutterDiv >= 4 && !busHoldStop) {
                        val bpm = drumEngine.bpm.coerceIn(40f, 240f)
                        val period = ((sampleRate * 240.0) / (bpm * busStutterDiv)).toInt().coerceAtLeast(64)
                        val pos = busStutterPhase % period
                        val fadeSamps = (sampleRate * 0.004f).toInt().coerceAtLeast(16).coerceAtMost(period / 4)
                        stutTarget = when {
                            pos < fadeSamps -> pos.toFloat() / fadeSamps
                            pos < period / 2 -> 1f
                            pos < period / 2 + fadeSamps -> 1f - (pos - period / 2).toFloat() / fadeSamps
                            else -> 0f
                        }
                        busStutterPhase++
                    } else {
                    ...
                    stutterGain += (stutTarget - stutterGain) * fadeCoeff
                    busMix *= (stutterGain * stopGain).coerceIn(0f, 1f)
                    val rawMaster = (frame.masterSample + busMix + micMon).coerceIn(-1.0f, 1.0f)
            ...
            for (t in 0 until looperTrackCount) {
                ...
                if (samples.isEmpty()) {
                    ...
                    FileOutputStream(pcm).use { fos ->
                        val bb = ByteBuffer.allocate(4 + samples.size * 4).order(ByteOrder.LITTLE_ENDIAN)
                        bb.putInt(samples.size)
