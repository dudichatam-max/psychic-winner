...
class SynthEngine(private val context: Context) {
    ...
    @Volatile var busPadX: Float = 0.5f
    @Volatile var busPadY: Float = 0.5f
    ...
    private inner class StemColor {
        ...
        fun process(input: Float, wah: Float, oct: Float, cho: Float): Float {
            ...
            if (smCho > 0.0008) {
                choPhase += 0.35 * (1.0 / sampleRate)
                if (choPhase >= 1.0) choPhase -= 1.0
                val lfo = kotlin.math.sin(choPhase * 6.283185307179586)
                choLp += (x - choLp) * (0.12 + 0.04 * lfo)
                val amt = 0.12 * smCho
                x = x * (1.0 - amt) + choLp * amt
            }
    ...
    private class SliceRoll {
        ...
        fun process(input: Float, armed: Boolean, div: Int, bpm: Float, sr: Int, wetCoeff: Float): Float {
            if (armed && div >= 4) {
    ...
    fun start() {
        ...
        Thread {
            ...
            val padFiltCoeff = 1f - kotlin.math.exp((-1.0 / (0.120 * sampleRate)).toFloat())
            val rollWetCoeff = 1f - kotlin.math.exp((-1.0 / (0.060 * sampleRate)).toFloat())
            ...
            while (isRunning) {
                ...
                for (i in 0 until bufferSize) {
                    ...
                    padCyFilt += (padCy - padCyFilt) * padFiltCoeff
                    ...
                    if (padWork) {
                        drumOutL = rollDrumL.process(drumL0, drumRollDiv >= 4, drumRollDiv, bpmNow, sampleRate, rollWetCoeff)
                        drumOutR = rollDrumR.process(drumR0, drumRollDiv >= 4, drumRollDiv, bpmNow, sampleRate, rollWetCoeff)
                        loopOutL = rollLoopL.process(pcmLoopL, rollOn && padTargetLoop, busStutterDiv, bpmNow, sampleRate, rollWetCoeff)
                        loopOutR = rollLoopR.process(pcmLoopR, rollOn && padTargetLoop, busStutterDiv, bpmNow, sampleRate, rollWetCoeff)
                        micOutL = rollMicL.process(micL0, rollOn && padTargetMic, busStutterDiv, bpmNow, sampleRate, rollWetCoeff)
                        micOutR = rollMicR.process(micR0, rollOn && padTargetMic, busStutterDiv, bpmNow, sampleRate, rollWetCoeff)
                        drumOutL = shapePadStem(drumOutL, padTargetDrum, drumLp, drumWet)
                        drumOutR = shapePadStem(drumOutR, padTargetDrum, drumLpR, drumWetR)
                        loopOutL = shapePadStem(loopOutL, padTargetLoop, loopLp, loopWet)
                        loopOutR = shapePadStem(loopOutR, padTargetLoop, loopLpR, loopWetR)
                    ...
                    val livePadOn = padTargetKey && (busPadTouched || liveWet[0] > 0.0008f || (rollOn && padTargetKey) || rollLive.isHot())
                    if (livePadOn) {
                        liveOut = rollLive.process(liveOut, rollOn && padTargetKey, busStutterDiv, bpmNow, sampleRate, rollWetCoeff)
                        liveOut = shapePadStem(liveOut, true, liveLp, liveWet)
                    }
                    ...
                    var recH = 0
                    while (recH < looperHotN) {
                        val recTrack = looperTracks[looperHotIdx[recH]]
                        if (recTrack.isRecording) {
                            recTrack.pushSample(liveOut)
                        }
                        recH++
                    }
                    ...
                    val padLive = padTargetKey && (busPadTouched || liveWet[0] > 0.0008f)
                    val liveL = center * (if (padLive) (1f - padCx * 0.88f) else 1f)
                    val liveR = center * (if (padLive) (1f + padCx * 0.88f) else 1f)
