...
class LooperPcmTrack(private val sampleRate: Int, maxSeconds: Int = 30) {
    ...
    fun snapshotForRestore(): LooperTrackRestoreSnapshot {
        val n = padN.coerceIn(0, padMs.size)
        return LooperTrackRestoreSnapshot(
            ...
            padMs = if (n > 0) padMs.copyOf(n) else LongArray(0),
            padXs = if (n > 0) padXs.copyOf(n) else FloatArray(0),
            padYs = if (n > 0) padYs.copyOf(n) else FloatArray(0)
        )
    }
    ...
    fun restoreFromSnapshot(s: LooperTrackRestoreSnapshot) {
        ...
        loadFromSamples(s.samples)
        volume = s.volume
        applyPan(s.pan)
        padN = s.padN.coerceIn(0, padMs.size)
        padI = s.padI.coerceIn(0, padN)
        lastStampMs = s.lastStampMs
        if (padN > 0) {
            System.arraycopy(s.padMs, 0, padMs, 0, padN)
            System.arraycopy(s.padXs, 0, padXs, 0, padN)
            System.arraycopy(s.padYs, 0, padYs, 0, padN)
        }
        playPos = if (length > 0) s.playPos.coerceIn(0, length - 1) else 0
        if (s.wasPlaying) startPlayback()
    }
            ...
            while (isRunning) {
                ...
                for (i in 0 until bufferSize) {
                    ...
                    if (padWork) {
                        ...
                        if (padTargetDrum || drumRollDiv >= 4 || rollDrumL.isHot() || rollDrumR.isHot() ||
                            drumWet[0] > 0.0008f || drumWetR[0] > 0.0008f) {
                        ...
                        if (padTargetLoop || loopAutoOn || rollLoopL.isHot() || rollLoopR.isHot() ||
                            loopWet[0] > 0.0008f || loopWetR[0] > 0.0008f) {
                        ...
                        if (padTargetMic || rollMicL.isHot() || rollMicR.isHot() ||
                            micWet[0] > 0.0008f || micWetR[0] > 0.0008f) {
