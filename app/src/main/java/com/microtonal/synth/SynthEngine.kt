...
class SynthEngine(private val context: Context) {
    ...
    private class SliceRoll {
        ...
        fun process(input: Float, armed: Boolean, div: Int, bpm: Float, sr: Int): Float {
            ...
            val need = ((sr * 240.0) / (bpm * div)).toInt().coerceIn(64, sr * 2)
            if (buf.size < need) buf = FloatArray(sr * 2)
            if (div != lastDiv) {
                lastDiv = div
                len = need
                fill = 0
    ...
    private fun shapePadStem(
        ...
        wetArr[0] += (target - wetArr[0]) * 0.04f
        if (wetArr[0] < 0.0008f) {
            wetArr[0] = 0f
            lp[0] = input
            return input
        }
        val depth = busPadY.coerceIn(0f, 1f)
        val cutoff = (280f + (4200f + busLfoMod * 3800f) * (0.25f + depth)).coerceIn(180f, 12000f)
        val coeff = 1f - kotlin.math.exp((-2.0 * Math.PI * cutoff / sampleRate).toFloat())
        lp[0] += coeff * (input - lp[0])
        val amp = 1f + busLfoMod * depth * 0.7f
        val wet = (lp[0] * amp).coerceIn(-1.2f, 1.2f)
            ...
            while (isRunning) {
                ...
                for (i in buffer.indices) {
                    ...
                    if (busPadTouched && (padTargetMic || padTargetLoop || padTargetDrum)) {
                        ...
                        busLfoMod = kotlin.math.sin(busLfoPhase * (Math.PI * 2.0)).toFloat()
                    } else {
                        busLfoMod *= 0.995f
                    }
                    var liveOut = frame.masterSample
                    var drumOut = shapePadStem(drumSample, padTargetDrum, drumLp, drumWet)
                    var loopOut = shapePadStem(pcmLoopMix, padTargetLoop, loopLp, loopWet)
                    var micOut = shapePadStem(micPlay, padTargetMic, micLp, micWet)
                    val bpmNow = drumEngine.bpm.coerceIn(40f, 240f)
                    val rollOn = busStutterDiv >= 4 && !busHoldStop
                    liveOut = rollLive.process(liveOut, rollOn && padTargetKey, busStutterDiv, bpmNow, sampleRate)
                    drumOut = rollDrum.process(drumOut, rollOn && padTargetDrum, busStutterDiv, bpmNow, sampleRate)
                    loopOut = rollLoop.process(loopOut, rollOn && padTargetLoop, busStutterDiv, bpmNow, sampleRate)
                    micOut = rollMic.process(micOut, rollOn && padTargetMic, busStutterDiv, bpmNow, sampleRate)
                    val stopTarget = if (busHoldStop) 0f else 1f
                    stopGain += (stopTarget - stopGain) * fadeCoeff
                    if (padTargetKey) liveOut *= stopGain
                    if (padTargetDrum) drumOut *= stopGain
