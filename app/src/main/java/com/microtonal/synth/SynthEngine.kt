...
class NoteSlot {
    ...
    @Volatile var velocity: Float = 1.0f
    ...
    fun updateAndActivate(
        ...
        velocity = newVelocity.coerceIn(0.01f, 1.0f)
...
class SynthEngine(private val context: Context) {
    ...
    fun getActiveVoiceCount(): Int {
        var c = 0
        for (i in 0 until maxVoices) if (noteSlots[i].active) c++
        return c
    }

    fun getMaxVoices(): Int = maxVoices
    ...
    @Volatile var looperVelocity = 1.0f
    ...
    fun noteOn(
        ...
        if (slot != null) {
            ...
            slot.velocity = velocity.coerceIn(0.01f, 1.0f)
            slot.active = true
