        ...
        Thread {
            ...
            while (isRunning) {
                ...
                for (i in buffer.indices) {
                    ...
                    if (padFlipOn) {
                        val cutoff = 180f + busPadX * 7800f
                        val coeff = 1f - kotlin.math.exp((-2.0 * Math.PI * cutoff / sampleRate).toFloat())
                        busLpZ += coeff * (busMix - busLpZ)
                        busHpZ += coeff * (busMix - busHpZ)
                        val filtered = busLpZ + (busMix - busHpZ) * (busPadY * 0.85f)
                        val resBoost = 1f + busPadY * 0.55f
                        busMix = (filtered * resBoost).coerceIn(-1.4f, 1.4f)
                    }
                    if (busHoldStop) {
                        busMix = 0f
                    } else if (busStutterDiv >= 4) {
                        val bpm = drumEngine.bpm.coerceIn(40f, 240f)
                        val period = ((sampleRate * 240.0) / (bpm * busStutterDiv)).toInt().coerceAtLeast(32)
                        val half = period / 2
                        if ((busStutterPhase % period) >= half) busMix = 0f
                        busStutterPhase++
                    } else {
                        busStutterPhase = 0
                    }
