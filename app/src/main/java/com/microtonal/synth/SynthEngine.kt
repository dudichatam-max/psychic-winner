...
class NoteSlot {
    ...
    var zdfCachedCutoff: Float = Float.NaN
    var zdfCachedRes: Float = Float.NaN
    var zdfG: Double = 0.0
    var zdfK: Double = 0.0
    var zdfH: Double = 0.0
...
class SynthEngine(private val context: Context) {
    ...
    private fun shapePadStem(
        ...
        val lfo = busLfoMod
        val d = if (padCy < 0f) -padCy else 0f
        val downAmt = (d / 0.18f).coerceIn(0f, 1f)
        val dCurve = d * d * d
        val wc = 0.070f * (1f - dCurve) + 0.0045f * dCurve
        lp[0] += wc * (input - lp[0]) * downAmt
        val dark = lp[0] * (1f - dCurve * 0.92f * downAmt)
        val tone = input * (1f - downAmt) + dark * downAmt
        val eqMix = if (wetArr[0] > dCurve * downAmt) wetArr[0] else dCurve * downAmt
        val mixed = input * (1f - eqMix) + tone * eqMix
        val absY = if (padCy >= 0f) padCy else -padCy
        val depth = if (!want || absY < 0.25f) {
            0f
        } else if (padCy < 0f) {
            ((absY - 0.25f) / 0.75f) * 0.55f
        } else {
            ((absY - 0.25f) / 0.75f) * 0.95f
        }
    ...
    fun start() {
        ...
        Thread {
            ...
            while (isRunning) {
                ...
                // Block-level snapshot: UI flags / BPM / roll routing are stable
                // for this 512-sample buffer. Sample-accurate DSP stays inside
                // the loop (phase, ADSR, ZDF state, delay/reverb, pad slew).
                val blkPadKey = padTargetKey
                val blkPadMic = padTargetMic
                val blkPadLoop = padTargetLoop
                val blkPadDrum = padTargetDrum
                val blkPadAny = blkPadKey || blkPadMic || blkPadLoop || blkPadDrum
                val blkPadTouched = busPadTouched
                val blkHoldStop = busHoldStop
                val blkDrumHoldStop = drumHoldStop
                val blkBusStutterDiv = busStutterDiv
                val blkBpmNow = drumEngine.bpm.coerceIn(40f, 240f)
                val blkRollOn = blkBusStutterDiv >= 4 && !blkHoldStop
                val blkDrumRollDiv = when {
                    drumStutterDiv >= 4 && !blkDrumHoldStop -> drumStutterDiv
                    blkRollOn && blkPadDrum -> blkBusStutterDiv
                    else -> 0
                }
                val blkWah = padWah
                val blkOct = padOct
                val blkCho = padCho
                val blkFxAmt = blkWah + blkOct + blkCho
                val blkLoopAutoOn = loopAutoOn
                val blkExtPlaying = dspEngine.isExternalAudioPlaying
                val blkPadCoeff = if (blkPadTouched) padCoeffAtk else padCoeffRel
                val blkLfoInc = padLfoHz / sampleRate
                val blkLfoRun = blkPadAny && (blkPadTouched ||
                    liveWet[0] > 0.002f || drumWet[0] > 0.002f ||
                    loopWet[0] > 0.002f || micWet[0] > 0.002f)
                val blkPadWork = blkRollOn || blkDrumRollDiv >= 4 || blkPadTouched ||
                ...
                val blkDrumWork = blkPadDrum || blkDrumRollDiv >= 4 ||
                    rollDrumL.isHot() || rollDrumR.isHot() ||
                    drumWet[0] > 0.0008f || drumWetR[0] > 0.0008f
                val blkLoopWork = blkPadLoop || blkLoopAutoOn ||
                    rollLoopL.isHot() || rollLoopR.isHot() ||
                    loopWet[0] > 0.0008f || loopWetR[0] > 0.0008f
                val blkMicWork = blkPadMic ||
                    rollMicL.isHot() || rollMicR.isHot() ||
                    micWet[0] > 0.0008f || micWetR[0] > 0.0008f
                val blkStopTarget = if (blkHoldStop) 0f else 1f
                val blkDrumStopTarget = if (blkDrumHoldStop || (blkHoldStop && blkPadDrum)) 0f else 1f
                val blkLivePanW = if (blkPadKey) 1f else 0f
                val blkCenter = 0.70710677f
                val blkFxDrum = blkFxAmt > 0.002f && blkPadDrum
                val blkFxLoop = blkFxAmt > 0.002f && blkPadLoop
                val blkFxMic = blkFxAmt > 0.002f && blkPadMic
                for (i in 0 until bufferSize) {
                    ...
                    smoothModX += (busPadX - smoothModX) * blkPadCoeff
                    smoothModY += (busPadY - smoothModY) * blkPadCoeff
                    padCx = (smoothModX * 2f - 1f).coerceIn(-1f, 1f)
                    padCy = (smoothModY * 2f - 1f).coerceIn(-1f, 1f)
                    if (blkLfoRun) {
                        busLfoPhase += blkLfoInc
                    ...
                    var micOutR: Float
                    if (blkPadWork) {
                        liveOut = rollLive.process(frame.masterSample, blkRollOn && blkPadKey, blkBusStutterDiv, blkBpmNow, sampleRate)
                        liveOut = shapePadStem(liveOut, blkPadKey, liveLp, liveWet)
                        if (blkDrumWork) {
                            drumOutL = rollDrumL.process(drumL0, blkDrumRollDiv >= 4, blkDrumRollDiv, blkBpmNow, sampleRate)
                            drumOutR = rollDrumR.process(drumR0, blkDrumRollDiv >= 4, blkDrumRollDiv, blkBpmNow, sampleRate)
                            drumOutL = shapePadStem(drumOutL, blkPadDrum, drumLp, drumWet)
                            drumOutR = shapePadStem(drumOutR, blkPadDrum, drumLpR, drumWetR)
                            if (blkPadDrum) {
                        ...
                        if (blkLoopWork) {
                            loopOutL = rollLoopL.process(pcmLoopL, blkRollOn && blkPadLoop, blkBusStutterDiv, blkBpmNow, sampleRate)
                            loopOutR = rollLoopR.process(pcmLoopR, blkRollOn && blkPadLoop, blkBusStutterDiv, blkBpmNow, sampleRate)
                            val savedCx = padCx
                            val savedCy = padCy
                            if (blkLoopAutoOn) {
                                padCx = loopAutoCx
                            ...
                            loopOutL = shapePadStem(loopOutL, blkPadLoop || blkLoopAutoOn, loopLp, loopWet)
                            loopOutR = shapePadStem(loopOutR, blkPadLoop || blkLoopAutoOn, loopLpR, loopWetR)
                            if (blkPadLoop || blkLoopAutoOn) {
                        ...
                        if (blkMicWork) {
                            micOutL = rollMicL.process(micL0, blkRollOn && blkPadMic, blkBusStutterDiv, blkBpmNow, sampleRate)
                            micOutR = rollMicR.process(micR0, blkRollOn && blkPadMic, blkBusStutterDiv, blkBpmNow, sampleRate)
                            micOutL = shapePadStem(micOutL, blkPadMic, micLp, micWet)
                            micOutR = shapePadStem(micOutR, blkPadMic, micLpR, micWetR)
                            if (blkPadMic) {
                    ...
                    if (blkFxDrum) {
                        drumOutL = colorDrumL.process(drumOutL, blkWah, blkOct, blkCho)
                        drumOutR = colorDrumR.process(drumOutR, blkWah, blkOct, blkCho)
                    }
                    if (blkFxLoop) {
                        loopOutL = colorLoopL.process(loopOutL, blkWah, blkOct, blkCho)
                        loopOutR = colorLoopR.process(loopOutR, blkWah, blkOct, blkCho)
                    }
                    if (blkFxMic) {
                        micOutL = colorMicL.process(micOutL, blkWah, blkOct, blkCho)
                        micOutR = colorMicR.process(micOutR, blkWah, blkOct, blkCho)
                    }
                    stopGain += (blkStopTarget - stopGain) * fadeCoeff
                    drumStopGain += (blkDrumStopTarget - drumStopGain) * fadeCoeff
                    if (blkPadKey) liveOut *= stopGain
                    ...
                    val liveL = blkCenter * (1f - padCx * 0.88f * blkLivePanW)
                    val liveR = blkCenter * (1f + padCx * 0.88f * blkLivePanW)
                    var rawL = liveOut * liveL + drumOutL + loopOutL + micOutL + micMon * blkCenter
                    var rawR = liveOut * liveR + drumOutR + loopOutR + micOutR + micMon * blkCenter
                    val absL = abs(rawL)
                    val absR = abs(rawR)
            ...
            while (isLoopPlaying) {
                ...
                for (i in 0 until maxVoices) {
                    if (noteSlots[i].isLooperNote) {
                        setSlotActive(noteSlots[i], false)
                    }
                }
