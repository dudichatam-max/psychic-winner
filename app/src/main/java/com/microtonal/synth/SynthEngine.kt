...
class SynthEngine(private val context: Context) {
    ...
    private fun shapePadStem(
        ...
        val target = if (want && (busPadTouched || away > 0.03f)) 1f else 0f
        wetArr[0] += (target - wetArr[0]) * if (busPadTouched) 0.006f else 0.0035f
        if (wetArr[0] < 0.0008f) {
            wetArr[0] = 0f
            lp[0] = input
            return input
        ...
        val lfo = busLfoMod
        val lfo01 = 0.5f + 0.5f * lfo
        val wc = (0.028f + lfo01 * (0.035f + (padCy + 1f) * 0.11f)).coerceIn(0.018f, 0.32f)
        lp[0] += wc * (input - lp[0])
        val bass = lp[0]
        val air = input - lp[0]
        var shaped: Float
        if (padCy >= 0f) {
            val hi = padCy
            shaped = input * (1f - hi * 0.18f) + air * (hi * (0.55f + lfo * 0.50f)) + bass * (1f - hi * 0.28f)
        } else {
            val d = -padCy
            val pulse = 0.42f + 0.58f * lfo01
            shaped = bass * (0.70f + d * 0.40f) * pulse + input * (1f - d * 0.38f)
        }
        shaped *= 1f + kotlin.math.min(0f, padCy) * 0.20f
        val aAbs = if (shaped >= 0f) shaped else -shaped
        if (aAbs > 0.72f) {
            val sgn = if (shaped >= 0f) 1f else -1f
            shaped = sgn * (0.72f + (aAbs - 0.72f) * 0.28f)
        }
        if (shaped > 0.92f) shaped = 0.92f else if (shaped < -0.92f) shaped = -0.92f
        return input * (1f - wetArr[0]) + shaped * wetArr[0]
    }
    ...
    fun start() {
        ...
        Thread {
            ...
            val padCoeffAtk = 1f - kotlin.math.exp((-1.0 / (0.022 * sampleRate)).toFloat())
            val padCoeffRel = 1f - kotlin.math.exp((-1.0 / (0.160 * sampleRate)).toFloat())
            ...
            while (isRunning) {
                ...
                val padLfoHz = 0.22 + smoothModY.coerceIn(0f, 1f) * 6.4
                var looperHotN = 0
                var ht = 0
                ...
                dspEngine.liveWahAmt = padWah
                dspEngine.liveOctAmt = padOct
                dspEngine.liveChoAmt = padCho
                ...
                for (i in 0 until bufferSize) {
                    ...
                    if (padWork) {
                        ...
                        if (padTargetDrum) {
                            val w = if (drumWet[0] > drumWetR[0]) drumWet[0] else drumWetR[0]
                            if (padCx > 0f) drumOutL *= 1f - padCx * 0.55f * w
                            if (padCx < 0f) drumOutR *= 1f + padCx * 0.55f * w
                        }
                        if (padTargetLoop) {
                            val w = if (loopWet[0] > loopWetR[0]) loopWet[0] else loopWetR[0]
                            if (padCx > 0f) loopOutL *= 1f - padCx * 0.55f * w
                            if (padCx < 0f) loopOutR *= 1f + padCx * 0.55f * w
                        }
                        if (padTargetMic) {
                            val w = if (micWet[0] > micWetR[0]) micWet[0] else micWetR[0]
                            if (padCx > 0f) micOutL *= 1f - padCx * 0.55f * w
                            if (padCx < 0f) micOutR *= 1f + padCx * 0.55f * w
                        }
                    } else {
                    ...
                    val livePanW = if (padTargetKey) liveWet[0] else 0f
                    val liveL = center * (1f - padCx * 0.62f * livePanW)
                    val liveR = center * (1f + padCx * 0.62f * livePanW)
