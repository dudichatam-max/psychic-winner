...
class SynthEngine(private val context: Context) {
    ...
    private var padYDown = false
    private var padYCurve = 0f
    private var padYWc = 0.10f
    private var padYDepth = 0f
    ...
    private fun shapePadStem(
        ...
        val eqMix: Float
        if (padYDown) {
            lp[0] += padYWc * (input - lp[0])
            tone = lp[0] * (1f - padYCurve * 0.92f)
            eqMix = if (wetArr[0] > padYCurve) wetArr[0] else padYCurve
        } else {
            tone = input
            eqMix = wetArr[0]
        }
        val mixed = input * (1f - eqMix) + tone * eqMix
        val depth = if (!want) 0f else padYDepth
                ...
                for (i in 0 until bufferSize) {
                    ...
                    if ((i and 7) == 0) {
                        if (padCy < 0f) {
                            ...
                            padYDown = true
                            padYCurve = d * d * d
                            padYWc = 0.070f * (1f - padYCurve) + 0.0045f * padYCurve
                            val absY = d
                            padYDepth = if (absY < 0.25f) 0f else ((absY - 0.25f) / 0.75f) * 0.85f
                        } else {
                            padYDown = false
                            padYCurve = 0f
                            padYWc = 0.10f
                            padYDepth = if (padCy < 0.25f) 0f else ((padCy - 0.25f) / 0.75f) * 0.70f
                        }
                    }
