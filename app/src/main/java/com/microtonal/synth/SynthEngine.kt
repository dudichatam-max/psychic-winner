...
class SynthEngine(private val context: Context) {
    ...
    private fun shapePadStem(
        ...
        wetArr[0] += (target - wetArr[0]) * if (busPadTouched) 0.012f else 0.004f
        if (wetArr[0] < 0.0008f) {
            wetArr[0] = 0f
            lp[0] = input
            return input
        }
        lp[0] += 0.09f * (input - lp[0])
        val bass = lp[0]
        val air = input - lp[0]
        var shaped = if (padCy > 0f) {
            input + air * (padCy * 1.05f) - bass * (padCy * 0.32f)
        } else if (padCy < 0f) {
            val d = -padCy
            input + bass * (d * 0.95f) - air * (d * 0.42f)
        } else {
            input
        }
        val vol = if (padCy < 0f) (1f + padCy * 0.58f) else 1f
        val trem = 1f + busLfoMod * (0.06f + kotlin.math.abs(padCy) * 0.10f)
        shaped *= vol * trem
        if (shaped > 1f) shaped = 1f else if (shaped < -1f) shaped = -1f
    ...
    fun start() {
        ...
        Thread {
            ...
            while (isRunning) {
                ...
                val padLfoHz = 0.16 + smoothModY.coerceIn(0f, 1f) * 7.2
                var looperHotN = 0
                var ht = 0
                ...
                for (i in 0 until bufferSize) {
                    ...
                    if (padWork) {
                        ...
                        if (padTargetDrum) {
                            ...
                            if (padCx > 0f) drumOutL *= 1f - padCx * 0.85f * w
                            if (padCx < 0f) drumOutR *= 1f + padCx * 0.85f * w
                        }
                        if (padTargetLoop) {
                            val w = if (loopWet[0] > loopWetR[0]) loopWet[0] else loopWetR[0]
                            if (padCx > 0f) loopOutL *= 1f - padCx * 0.85f * w
                            if (padCx < 0f) loopOutR *= 1f + padCx * 0.85f * w
                        }
                        if (padTargetMic) {
                            val w = if (micWet[0] > micWetR[0]) micWet[0] else micWetR[0]
                            if (padCx > 0f) micOutL *= 1f - padCx * 0.85f * w
                            if (padCx < 0f) micOutR *= 1f + padCx * 0.85f * w
                        }
                    } else {
                    ...
                    val liveL = center * (1f - padCx * livePanW)
                    val liveR = center * (1f + padCx * livePanW)
