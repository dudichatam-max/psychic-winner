...
class SynthEngine(private val context: Context) {
    ...
    private fun shapePadStem(
        ...
        val wCoeff = if (target > 0.5f) 0.00016f else 0.00011f
        ...
        if (padCy < 0f) {
            val d = -padCy
            lp[0] += (0.018f + (1f - d) * 0.03f) * (input - lp[0])
            val vol = 1f - d * 0.92f
            val pulse = (1f - d * 0.55f) + d * 0.55f * lfo01
            shaped = lp[0] * vol * pulse
        } else {
            lp[0] += 0.12f * (input - lp[0])
            val depth = padCy * 0.42f
            shaped = input * (1f + lfo * depth)
        }
        if (shaped > 0.90f) shaped = 0.90f else if (shaped < -0.90f) shaped = -0.90f
    ...
    fun start() {
        ...
        Thread {
            ...
            while (isRunning) {
                ...
                val padLfoHz = 0.38 + smoothModY.coerceIn(0f, 1f) * 6.8
                var looperHotN = 0
                var ht = 0
