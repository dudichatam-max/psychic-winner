...
class DrumEngine(private val sampleRate: Int) {
    ...
    private fun rebuildStepIncrements() {
        val baseStepsPerSecond = (bpm / 60.0) * 4.0
        val swingAmt = swing.coerceIn(0f, 1f) * 0.5
        var i = 0
        while (i < 16) {
            val stretch = if (i % 2 == 0) (1.0 + swingAmt) else (1.0 - swingAmt)
            val stepsPerSecond = baseStepsPerSecond / stretch
            cachedStepIncrement[i] = stepsPerSecond / sampleRate
            i++
        }
    }
    ...
    fun processNextSample(): Float {
        ...
        stepPhase += cachedStepIncrement[currentStep]
