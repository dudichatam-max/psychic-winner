...
class DrumEngine(private val sampleRate: Int) {
    ...
    // Core playback state
    // ------------------------------------------------------------------
    val drumTrackCount = 8
    val drumSamples = arrayOfNulls<FloatArray>(8)
    val grid = Array(8) { BooleanArray(16) }
    val trackVolumes = FloatArray(8) { 1.0f }
    val trackPans = FloatArray(8) { 0f }
    val trackPanL = FloatArray(8) { 0.70710677f }
    val trackPanR = FloatArray(8) { 0.70710677f }
    val trackNames = arrayOf("Kick", "Snare", "Hi-Hat", "Perc", "Extra 1", "Extra 2", "Extra 3", "Extra 4")
    @Volatile var lastOutL: Float = 0f
    @Volatile var lastOutR: Float = 0f
    ...
    private val playIndices = IntArray(8) { -1 }
    private var stepPhase = 0.0
    private val cachedStepIncrement = DoubleArray(16)
    private var cachedBpm = Float.NaN
    private var cachedSwing = Float.NaN

    private fun refreshStepIncrements() {
        val b = bpm
        val s = swing
        if (b == cachedBpm && s == cachedSwing) return
        cachedBpm = b
        cachedSwing = s
        val baseStepsPerSecond = (b / 60.0) * 4.0
        val swingAmt = s.coerceIn(0f, 1f) * 0.5
        val invSr = 1.0 / sampleRate
        val evenInc = (baseStepsPerSecond / (1.0 + swingAmt)) * invSr
        val oddInc = (baseStepsPerSecond / (1.0 - swingAmt)) * invSr
        var i = 0
        while (i < 16) {
            cachedStepIncrement[i] = if ((i and 1) == 0) evenInc else oddInc
            i++
        }
    }
    ...
    data class DrumPattern(
        val grid: Array<BooleanArray> = Array(8) { BooleanArray(16) },
        val bpm: Float = 120f,
        val masterVolume: Float = 0.8f,
        val trackVolumes: FloatArray = FloatArray(8) { 1.0f },
        val trackPans: FloatArray = FloatArray(8) { 0f },
        val repeatCount: Int = 0
    ) {
        fun deepCopy(): DrumPattern = DrumPattern(
            grid = Array(8) { t -> if (t < this.grid.size) this.grid[t].copyOf(16) else BooleanArray(16) },
            bpm = this.bpm,
            masterVolume = this.masterVolume,
            trackVolumes = FloatArray(8) { t -> if (t < this.trackVolumes.size) this.trackVolumes[t] else 1.0f },
            trackPans = FloatArray(8) { t -> if (t < this.trackPans.size) this.trackPans[t] else 0f },
    ...
    val patternRepeat = IntArray(8) { 0 }
    private var patternCyclesPlayed = 0
    private var playGain = 0f
    private var playGainTarget = 0f
    private var pendingRewind = false
    private val playFadeCoeff = (1.0 / (sampleRate * 0.012)).toFloat()
    ...
    fun loadPattern(index: Int) {
        ...
        for (t in 0 until drumTrackCount) {
            if (t < p.grid.size) System.arraycopy(p.grid[t], 0, grid[t], 0, 16)
            else for (s in 0 until 16) grid[t][s] = false
            trackVolumes[t] = if (t < p.trackVolumes.size) p.trackVolumes[t] else 1.0f
            setTrackPan(t, if (t < p.trackPans.size) p.trackPans[t] else 0f)
        }
        bpm = p.bpm
        masterVolume = p.masterVolume
        currentPatternIndex = index
        patternCyclesPlayed = 0
    }
    ...
    fun saveCurrentToPattern(index: Int = currentPatternIndex) {
        if (index !in 0 until 8) return
        val g = Array(drumTrackCount) { t -> grid[t].copyOf() }
        patterns[index] = DrumPattern(
            grid = g,
            bpm = bpm,
            masterVolume = masterVolume,
            trackVolumes = trackVolumes.copyOf(),
            trackPans = trackPans.copyOf(),
            repeatCount = if (index in 0 until 8) patternRepeat[index] else 0
        )
        currentPatternIndex = index
    ...
    fun loadKit(index: Int, context: Context, startPattern: Int = 0) {
        ...
        for (i in 0 until 8) {
            ...
            patternRepeat[i] = patterns[i].repeatCount
        }
    ...
    fun setSample(trackIndex: Int, pcm: FloatArray) {
        if (trackIndex in 0 until drumTrackCount) {
            drumSamples[trackIndex] = pcm
            playIndices[trackIndex] = -1
        }
    }

    fun setPatternRepeat(index: Int, count: Int) {
        if (index !in 0 until 8) return
        val v = when {
            count < 0 -> -1
            count > 10 -> 10
        ...
        patternRepeat[index] = v
        val p = patterns[index]
        patterns[index] = DrumPattern(p.grid, p.bpm, p.masterVolume, p.trackVolumes, p.trackPans, v)
        if (currentKitIndex in 0 until 8) {
            kits[currentKitIndex].patterns[index] = patterns[index].deepCopy()
        }
        if (index == currentPatternIndex) patternCyclesPlayed = 0
    }

    fun setTrackPan(trackIndex: Int, pan: Float) {
        if (trackIndex !in 0 until drumTrackCount) return
        val p = pan.coerceIn(-1f, 1f)
        trackPans[trackIndex] = p
        val a = (p + 1f) * 0.7853982f
        trackPanL[trackIndex] = kotlin.math.cos(a)
        trackPanR[trackIndex] = kotlin.math.sin(a)
    }
    ...
    fun processNextSample(): Float {
        ...
        if (pendingRewind && playGain <= 0.0008f) {
            isPlaying = false
            pendingRewind = false
            currentStep = 0
            stepPhase = 0.0
            for (t in 0 until drumTrackCount) playIndices[t] = -1
            lastOutL = 0f
            lastOutR = 0f
            return 0f
        }
        ...
        // Base 16th + swing stretch are cached per step. Rebuild only when bpm/swing change.
        refreshStepIncrements()
        val stepIdx = currentStep
        if (stepIdx >= 0 && stepIdx < 16) {
            stepPhase += cachedStepIncrement[stepIdx]
        }
        ...
        for (t in 0 until drumTrackCount) {
            ...
            if (idx >= 0 && sample != null) {
                if (idx < sample.size) {
                    val v = sample[idx] * trackVolumes[t]
                    mixedL += v * trackPanL[t]
                    mixedR += v * trackPanR[t]
                    playIndices[t] = idx + 1
                } else {
                    playIndices[t] = -1
                }
        ...
        val mv = masterVolume * playGain
        lastOutL = (mixedL * mv).coerceIn(-1f, 1f)
        lastOutR = (mixedR * mv).coerceIn(-1f, 1f)
        return ((lastOutL + lastOutR) * 0.5f).coerceIn(-1f, 1f)
    }

    /** In-place copy for the audio thread. Does not allocate grids or kit snapshots. */
    private fun flushWorkingPatternInPlace(index: Int) {
        if (index !in 0 until 8) return
        val p = patterns[index]
        var t = 0
        while (t < drumTrackCount) {
            if (t < p.grid.size) {
                val src = grid[t]
                val dst = p.grid[t]
            ...
            if (t < p.trackVolumes.size) p.trackVolumes[t] = trackVolumes[t]
            if (t < p.trackPans.size) p.trackPans[t] = trackPans[t]
            t++
        }
    }

    private fun advancePatternChain() {
        val idx = currentPatternIndex.coerceIn(0, 7)
        val rep = patternRepeat[idx]
        if (rep == 0 || rep == -1) return
        patternCyclesPlayed++
        if (patternCyclesPlayed < rep) return
        patternCyclesPlayed = 0
        var next = (idx + 1) % 8
        var hops = 0
        while (hops < 8 && patternRepeat[next] == 0) {
            next = (next + 1) % 8
            hops++
        }
        if (patternRepeat[next] == 0 || next == idx) return
        flushWorkingPatternInPlace(idx)
        loadPattern(next)
    }


    /**
     * Stop playback and rewind the sequencer to the first step of the beat.
     * Does not change the grid, kit, BPM, swing or volumes.
     */
    fun stopAndRewind() {
        playGainTarget = 0f
        pendingRewind = true
    ...
    /**
     * Start playback from step 0 and trigger any hits that sit on the first step.
     */
    fun startFromBeginning() {
        ...
        for (t in 0 until drumTrackCount) {
            playIndices[t] = -1
            if (grid[t][0] && drumSamples[t] != null) {
                playIndices[t] = 0
            }
        }
        playGain = 0f
    ...
    fun writeKitBundleToDir(context: Context, dest: File): Boolean {
        return try {
            saveCurrentKit(currentKitIndex, context)
            if (!dest.exists()) dest.mkdirs()
            File(dest, "kits.json").writeText(buildKitsMetadataJson().toString())
            val srcDir = File(context.filesDir, "drum_kits")
            if (srcDir.exists()) {
                val files = srcDir.listFiles() ?: emptyArray()
                for (f in files) {
                    if (f.isFile && f.name.endsWith(".pcm")) {
                        f.inputStream().use { ins ->
                            File(dest, f.name).outputStream().use { outs -> ins.copyTo(outs) }
                        }
                    }
                }
            }
    ...
    fun loadKitBundleFromDir(context: Context, src: File): Boolean {
        return try {
            if (!src.exists()) return false
            val meta = File(src, "kits.json")
            if (meta.exists()) {
                applyKitsMetadataJson(JSONObject(meta.readText()))
            }
            val kitDir = File(context.filesDir, "drum_kits")
            if (!kitDir.exists()) kitDir.mkdirs()
            val files = src.listFiles() ?: emptyArray()
            for (f in files) {
                if (f.isFile && f.name.endsWith(".pcm")) {
                    f.inputStream().use { ins ->
                        File(kitDir, f.name).outputStream().use { outs -> ins.copyTo(outs) }
                    }
                }
            }
                    ...
                    while (entry != null) {
                        ...
                        if (name == "kits.json") {
                            ...
                            if (parts.size == 2) {
                                ...
                                if (k != null && t != null && k in 0..7 && t in 0..7) {
        ...
        for (k in 0 until 8) {
            ...
            for (p in 0 until 8) {
                ...
                for (t in 0 until 8) {
                    ...
                    val row = if (t < pat.grid.size) pat.grid[t] else BooleanArray(16)
                    for (s in 0 until 16) {
                        sb.append(if (row[s]) '1' else '0')
                    }
                ...
                val tpArr = JSONArray()
                for (t in 0 until 8) {
                    tvArr.put((if (t < pat.trackVolumes.size) pat.trackVolumes[t] else 1.0f).toDouble())
                    tpArr.put((if (t < pat.trackPans.size) pat.trackPans[t] else 0f).toDouble())
                }
                pObj.put("trackVolumes", tvArr)
                pObj.put("trackPans", tpArr)
                pObj.put("repeatCount", pat.repeatCount)
    ...
    private fun applyKitsMetadataJson(root: JSONObject) {
        ...
        for (k in 0 until minOf(8, kitsArr.length())) {
            ...
            for (p in 0 until minOf(8, patternsArr.length())) {
                ...
                val g = Array(8) { BooleanArray(16) }
                if (gridArr != null) {
                    for (t in 0 until minOf(8, gridArr.length())) {
                ...
                val tvs = FloatArray(8) { 1.0f }
                if (tvArr != null) {
                    for (t in 0 until minOf(8, tvArr.length())) {
                        tvs[t] = tvArr.optDouble(t, 1.0).toFloat()
                    }
                }
                val tpArr = pObj.optJSONArray("trackPans")
                val tps = FloatArray(8) { 0f }
                if (tpArr != null) {
                    for (t in 0 until minOf(8, tpArr.length())) {
                        tps[t] = tpArr.optDouble(t, 0.0).toFloat().coerceIn(-1f, 1f)
                    }
                }
                val rep = pObj.optInt("repeatCount", 0).let { r ->
                    when {
                        r < 0 -> -1
                        r > 10 -> 10
                        else -> r
                ...
                kits[k].patterns[p] = DrumPattern(g, bpm, master, tvs, tps, rep)
            }
        }
    }
