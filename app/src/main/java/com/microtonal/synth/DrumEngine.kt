    ...
    private fun decodeAiff(context: Context, uri: Uri): FloatArray? {
        return try {
            context.contentResolver.openInputStream(uri)?.use { input ->
                ...
                var isLittleEndian = false // AIFF is big-endian; some AIFC variants differ but we stay strict
                ...
                while (true) {
                    ...
                    if (chunkSize < 0 || chunkSize > 100_000_000) break // safety

                    when (chunkId) {
                        "COMM" -> {
                            val comm = ByteArray(chunkSize.coerceAtMost(64))
                            val got = input.read(comm)
                            if (got < 18) return null
                            val bb = ByteBuffer.wrap(comm).order(ByteOrder.BIG_ENDIAN)
                            numChannels = bb.short.toInt().coerceIn(1, 2)
                            numSampleFrames = bb.int
                            sampleSize = bb.short.toInt()
                            // 80-bit IEEE extended → double
                            fileSampleRate = extended80ToDouble(comm, 8)
                            // For AIFC there may be compression type after the rate
                            if (formType == "AIFC" && got >= 22) {
                                val comp = String(comm, 18, 4, Charsets.US_ASCII)
                                if (comp != "NONE" && comp != "raw " && comp != "twos") {
                                    // Compressed AIFC not supported
                                    return null
                                }
                            }
                            // Skip remaining bytes of chunk if any
                            val remaining = chunkSize - got
                            if (remaining > 0) input.skip(remaining.toLong())
                        }
                        "SSND" -> {
                            // offset (4) + blockSize (4) + data
                            ...
                            soundData = ByteArray(dataSize)
                            var pos = 0
                            while (pos < dataSize) {
                                val r = input.read(soundData!!, pos, dataSize - pos)
                                if (r <= 0) break
                                pos += r
                            }
                            if (pos < dataSize) soundData = soundData!!.copyOf(pos)
                            // AIFF chunks are even-padded
                            if (chunkSize % 2 != 0) input.skip(1)
                            // We can stop after SSND (most files have it last)
                            break
                        }
                        else -> {
                            // Skip unknown chunk (+ pad byte if odd size)
                ...
                val expectedBytes = numSampleFrames * numChannels * bytesPerSample
                val available = minOf(soundData.size, expectedBytes)
                val frames = available / (numChannels * bytesPerSample)
                if (frames <= 0) return null

                val raw = FloatArray(frames)
                val bb = ByteBuffer.wrap(soundData, 0, available).order(ByteOrder.BIG_ENDIAN)
                ...
                for (i in 0 until frames) {
                    ...
                    for (ch in 0 until numChannels) {
                        val v = when (sampleSize) {
                            ...
                            24 -> {
                                ...
                                var s = (b0 shl 16) or (b1 shl 8) or b2
                                if (s and 0x800000 != 0) s = s or -0x1000000 // sign extend
                                s / 8388608f
                            }
                    ...
                    raw[i] = if (numChannels > 1) sample / numChannels else sample
                }

                return resampleToEngineRate(raw, frames, fileSampleRate.toInt().coerceAtLeast(8000))
            }
        } catch (e: Exception) {
            e.printStackTrace()
            null
